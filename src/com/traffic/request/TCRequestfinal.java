...
public class TCRequestfinal extends MyRequest{
	...
	private void setParamar(){
		...
		parameters.add(new BasicNameValuePair("ssid","0"));
		parameters.add(new BasicNameValuePair("from",getPram("from", prams)));
//		parameters.addAll(ssidParameters);
	...
	public String getPram(String key,String[] pramers){
		for (String parString : pramers) {
			if(parString.contains(key)){
				return parString.split("=")[1];
			}
		}
		return null;
	}
	...
	public String[] getPrams(){
		Elements es = doc.select("[order=5]").select("a");
		if(es==null||es.size()==0){
			es = doc.select("[order=6]").select("a");
		}
